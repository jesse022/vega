package com.sanlux.pay.allinpay.trans;
 ...
 */
@Data
public class AllinpayTrans implements Trans, Serializable {
     ...
     * 金额转换成内部的单位计算（单位：分）
     */
    public Long serviceFeeToFen() {
        if (serviceFee == null) return 0L;
        BigDecimal money = new BigDecimal(serviceFee);
        return money.multiply(ratio).longValue();
    }
     ...
     * 金额转换成内部的单位计算（单位：分）
     */
    public Long settlementFeeToFen() {
        if (settlementFee == null) return 0L;
        BigDecimal money = new BigDecimal(settlementFee);
        return money.multiply(ratio).longValue();
    }
