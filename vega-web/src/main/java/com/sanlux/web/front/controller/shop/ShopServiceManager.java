...
/**
 * 店铺业务经理信息管理Controller
 *
 * Created by lujm on 2017/8/8.
 */
@RestController
@Slf4j
@RequestMapping("/api/shop/service/manager")
public class ShopServiceManager {
    ...
    public Long createServiceManagerUser(@RequestParam String mobile,
        ...
        serviceManager.setShopId(currentShopId);
        serviceManager.setShopName(getShopById(currentShopId).getName());
        serviceManager.setType(type);
        ...
        if (!optionalResponse.isSuccess()) {
            log.error("failed to find service manager by id = {}, cause : {}", resp.getResult(), optionalResponse.getError());
        }
    ...
    public Paging<ServiceManager> vegaUserPaging(ServiceManagerCriteria criteria) {
        ...
        if(Objects.nonNull(criteria.getId())) {
            if (!Strings.isNullOrEmpty(criteria.getId().toString())) {
                ...
                if(userResp.getResult().isPresent() && Objects.equals(currentShopId, userResp.getResult().get().getShopId())) {
        ...
        criteria.setShopId(currentShopId);// 塞入所属店铺Id
    ...
    public ServiceManager findById(@PathVariable(value = "id") Long id) {
        ...
        if(resp.getResult().isPresent() && Objects.equals(currentShopId, resp.getResult().get().getShopId())){
